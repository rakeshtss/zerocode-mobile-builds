...
$(document).ready(function () {
  ...
  function getChatList() {
    ...
    $.ajax({
      url: url + 'metamorphosis/api/discussion_questions/list/discussion-questions-list',
      type: 'POST',
          ...
          if (chat_list && chat_list.length) {
            $.each(chat_list, function (i, o) {
             var activeStatus = moment(new Date(o.created_time)).fromNow();
              ...
              let div = $('<div>', { 'class': 'tab-accords-body-left' }).append('<img src="assets/themes/metamorphosis/images/user.png">');
                ...
                text: 'Post', class: 'btn btn-sm btn-primary btn-submit', click: function () {
                  if($('#chat_box_' + i).val().length){
                    $.ajax({
                      ...
                      url: url + 'metamorphosis/api/discussion_reply/save-update/discussion-reply-save',
                      type: 'POST',
                      dataType: 'json', // added data type
                      headers: header,
              ...
              chatListHeader = `<div class="tab-accords-header-left">
                <div class="tab-accords-header-left-img">
                  <img src="assets/themes/metamorphosis/images/user.png">
                </div>
              </div>
  ...
  $('#submitComment').click(function () {
    ...
    if( $('#commentBox').val().length){
      $.ajax({
        // url: url+'/api/discussion_questions/list/discussion-questions-list',
        url: url + 'metamorphosis/api/discussion_questions/save-update/discussion-questions-save-update',
        type: 'POST',
        dataType: 'json', // added data type
        headers: header,
