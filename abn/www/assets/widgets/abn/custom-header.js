...
var zc;
zc.editionTitle = zc.params.module.replace('_', ' ');
...
$(document).ready(function() {
  ...
  //     document.getElementById('divEditionSelector').style.display = "flex";
  // }
      ...
      try {
        ...
        for (var i = 0; i < 7; i++) {
          ...
          editions.forEach(item => {
            ...
            if (eDate.getTime() === todayDate.getTime()) {
              var eCategory = item.sub_sub_category.code
                ? item.sub_sub_category.code
        ...
        $('#datepicker').datepicker({
          ...
          onSelect: function() {
            ...
            var eCategory = editionDetails.sub_sub_category.code
              ? editionDetails.sub_sub_category.code
              : editionDetails.sub_category.code;
            // window.location.replace("/" + editionDetails.uid_actual + "/" + eCategory + "/" + editionDetails.date.split("-").reverse().join("-"))
            var turl =
              '/' +
              editionDetails.uid_actual +
              '/' +
              eCategory +
              '/' +
              editionDetails.date
...
$(document).on('click.dropdown', '.dropdown.keep-inside-clicks-open', function(
  e
) {
  e.stopPropagation();
});
